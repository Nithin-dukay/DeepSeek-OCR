...
if __name__ == "__main__":
    ...
    if '<image>' in args.prompt:
        print("\nPreprocessing image with selected mode...")
    ...
    # Run inference
    result_out = asyncio.run(stream_generate(image_features, prompt, args.mode))

    # Save results
    if args.save_results and '<image>' in prompt:
        ...
        output_file = f'{OUTPUT_PATH}/result_{args.mode}_ori.mmd'
        with open(output_file, 'w', encoding='utf-8') as afile:
            afile.write(outputs)
        print(f"Saved original output to: {output_file}")
        ...
        output_file = f'{OUTPUT_PATH}/result_{args.mode}.mmd'
        with open(output_file, 'w', encoding='utf-8') as afile:
            afile.write(outputs)
        print(f"Saved processed output to: {output_file}")

        # Save image with bounding boxes
        result_image_file = f'{OUTPUT_PATH}/result_{args.mode}_with_boxes.jpg'
        result.save(result_image_file)
        print(f"Saved result image to: {result_image_file}")
