"""
Enhanced version of run_dpsk_ocr_pdf.py with command-line mode selection support.

Usage:
    python run_dpsk_ocr_pdf_with_mode.py --mode gundam
...
from config import MODEL_PATH, INPUT_PATH, OUTPUT_PATH, PROMPT, SKIP_REPEAT, MAX_CONCURRENCY, NUM_WORKERS
...
def process_single_image(args_tuple):
    """single image"""
    image, prompt, crop_mode = args_tuple
    cache_item = {
        "prompt": prompt,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='DeepSeek-OCR vLLM PDF Inference with Mode Selection',
...
Examples:
  ...
  python run_dpsk_ocr_pdf_with_mode.py --mode base --input document.pdf
    ...
    parser.add_argument(
        ...
        help='OCR mode to use (default: gundam)'
    )
...
if __name__ == "__main__":
    ...
    # List modes and exit if requested
    if args.list_modes:
    ...
    print(f"=" * 60)
    print(f"Running DeepSeek-OCR PDF with mode: {args.mode.upper()}")
    ...
    if not input_path or not os.path.exists(input_path):
        ...
        print("Please specify --input or set INPUT_PATH in config.py")
        exit(1)
    ...
    # Initialize LLM with mode-specific settings
    llm = LLM(
        model=MODEL_PATH,
    ...
    # Prepare batch inputs with mode-specific crop_mode
    with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:  
        batch_inputs = list(tqdm(
            executor.map(process_single_image, [(img, prompt, crop_mode) for img in images]),
            total=len(images),
