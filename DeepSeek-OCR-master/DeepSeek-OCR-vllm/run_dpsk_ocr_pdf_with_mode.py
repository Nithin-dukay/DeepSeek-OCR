"""
Example script for PDF processing with dynamic mode selection.

This script demonstrates batch PDF processing with different modes
(Tiny, Small, Base, Large, Gundam) using vLLM.
...
def process_single_image(args):
    """Process a single image with mode configuration."""
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*60}")
    print(f"DeepSeek-OCR PDF Processing")
    print(f"{'='*60}")
    ...
    # Initialize vLLM
    print(f'{Colors.BLUE}Initializing vLLM engine...{Colors.RESET}')
    llm = LLM(
        model=MODEL_PATH,
    ...
    # Load PDF
    print(f'{Colors.RED}Loading PDF: {args.pdf}{Colors.RESET}')
    images = pdf_to_images_high_quality(args.pdf)
    print(f'{Colors.GREEN}Loaded {len(images)} pages{Colors.RESET}')

    # Preprocess images with mode configuration
    print(f'\n{Colors.YELLOW}Preprocessing images with {args.mode} mode...{Colors.RESET}')
    with ThreadPoolExecutor(max_workers=args.num_workers) as executor:
        batch_inputs = list(tqdm(
            executor.map(process_single_image, [(img, args.prompt, mode_config) for img in images]),
            total=len(images),
            desc="Pre-processing"
        ))
    ...
    # Process outputs
    print(f'\n{Colors.GREEN}Processing outputs...{Colors.RESET}')
    mmd_det_path = os.path.join(args.output, f'{os.path.basename(args.pdf).replace(".pdf", "")}_{args.mode}_det.mmd')
    mmd_path = os.path.join(args.output, f'{os.path.basename(args.pdf).replace(".pdf", "")}_{args.mode}.mmd')
    pdf_out_path = os.path.join(args.output, f'{os.path.basename(args.pdf).replace(".pdf", "")}_{args.mode}_layouts.pdf')
    ...
    for output, img in tqdm(zip(outputs_list, images), total=len(images), desc="Processing pages"):
        ...
        result_image = process_image_with_refs(image_draw, matches_ref, jdx, args.output)
    ...
    # Save results
    print(f'\n{Colors.YELLOW}Saving results...{Colors.RESET}')
    ...
    print(f'  - {os.path.basename(mmd_det_path)} (original output)')
    print(f'  - {os.path.basename(mmd_path)} (processed output)')
    print(f'  - {os.path.basename(pdf_out_path)} (annotated PDF)')
    print(f'\nMode used: {args.mode.upper()}')
    print(f'  - base_size: {mode_config["base_size"]}')
    print(f'  - image_size: {mode_config["image_size"]}')
    print(f'  - crop_mode: {mode_config["crop_mode"]}')
